...
const LeaveTypeTable = ({ setLeaveTypeEdit }) => {
  ...
  const {
    ...
    queryKey: ["leavetype", onSearch, store.isSearch],
    queryFn: async ({ pageParam = 1 }) =>
      await queryDataInfinite(
        `/v2/leavetype/search`, // search endpoint
        `/v2/leavetype/page/${pageParam}`, // list endpoint
  ...
  const handleArchive = (item) => {
    setIsData(item.leave_type_name);
    dispatch(setIsArchive(true));
    setId(item.leave_type_aid);
    setIsArchiving(true);
    setIsRestore(false);
  };
    ...
    <>
      ...
      {store.isArchive && (
        <ModalArchive
          ...
          queryKey={"leavetype"}
          mysqlEndpoint={`/v2/leavetype/active/${id}`}
          item={isData}
          archive={isArchiving}
        />
      )}
